...
void ImuMadgwick::updateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
	float recipNorm;
	float s0, s1, s2, s3;
	float qDot1, qDot2, qDot3, qDot4;
...
float ImuMadgwick::invSqrt(float x) {
	float halfx = 0.5f * x;
	float y = x;
	long i = *(long*)&y;
	i = 0x5f3759df - (i>>1);
...
void ImuMadgwick::computeAngles()
{
...
void ImuMadgwick::imu_update() {
    int16_t accelX = read16bit(ACCEL_DATA_X1);
    int16_t accelY = read16bit(ACCEL_DATA_X1 + sizeof(int16_t));
    int16_t accelZ = read16bit(ACCEL_DATA_X1 + 2 * sizeof(int16_t));
    ...
    imu_measurement.gyro.z = gyroZ * GYRO_SENS_LSB_TO_DPS;
    imu_measurement.accel.x = accelX * ACCEL_SENS_LSB_TO_MS2;
    imu_measurement.accel.y = accelY * ACCEL_SENS_LSB_TO_MS2;
    imu_measurement.accel.z = accelZ * ACCEL_SENS_LSB_TO_MS2;
}
