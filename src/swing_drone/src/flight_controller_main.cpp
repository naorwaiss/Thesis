...
drone_mod mode_selector(){
    if (joy_data_read.aux_1 > 0.5){
        return drone_mod::backstepping;
    }
    else{
        return drone_mod::rate_mode;
    }
}
...
int main(int argc, char** argv) {
    ...
    imu_data_graber = std::make_shared<data_graber_ros>(&imu_data_read, &euler_angles_read, &arm_data_read, &joy_data_read);
