...
uint16_t channels_read[14];  // RC channel values
...
void onConnection(RTComSession& session) {
    ...
    session.on(EdgeTX, [session](const uint8_t *bytes, size_t size) {
        char data[size + 1] = {0};
        memcpy(data, bytes, size);
        Serial.println(data);

    });
    session.on(JOYSTICK, [session](const uint8_t *bytes, size_t size) {
        char data[size + 1] = {0};
        memcpy(data, bytes, size);
        Serial.println(data);

    });
    session.on(PLAYSTATION, [session](const uint8_t *bytes, size_t size) {
        char data[size + 1] = {0};
        memcpy(data, bytes, size);
        Serial.println(data);
