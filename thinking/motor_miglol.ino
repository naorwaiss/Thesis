...
void encoder_to_velo(unsigned long dt) {
  // Ensure dt is not zero to prevent division by zero
  if (dt == 0) {
    Serial.println("Error: dt is zero");
    return;
  }

  // Define encoder resolution (counts per revolution)
  ...
  float angular_velocity = delta_theta / dt_seconds;

  // Print encoder counts and angular velocity
  Serial.print("Encoder Count: ");
  Serial.println(position);
